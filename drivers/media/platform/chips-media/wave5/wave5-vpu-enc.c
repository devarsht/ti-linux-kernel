...
static void wave5_vpu_enc_start_encode(struct vpu_instance *inst)
{
	...
	src_vbuf = list_first_entry_or_null(&inst->avail_src_bufs,
					    struct vpu_buffer, list);
	dst_vbuf = list_first_entry_or_null(&inst->avail_dst_bufs,
					    struct vpu_buffer, list);
	...
	if (!src_vbuf) {
		...
		pic_param.pts = src_buf->vb2_buf.timestamp;
	}
